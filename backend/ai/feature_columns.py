"""132 ML feature column names aligned with the standard symptom–disease dataset schema."""

_RAW_HEADER = (
    "itching,skin_rash,nodal_skin_eruptions,dischromic _patches,continuous_sneezing,shivering,chills,"
...
def _build_feature_columns() -> list[str]:
    cols = [c.strip() for c in _RAW_HEADER.split(",") if c.strip() and c.strip() != "prognosis"]
    seen: set[str] = set()
    uniq: list[str] = []
    for c in cols:
        if c not in seen:
            seen.add(c)
            uniq.append(c)
    # Dataset duplicates removed → pad to 132 clinical features used by the app
    if "chest_tightness" not in seen:
        uniq.append("chest_tightness")
    if len(uniq) < 132:
        uniq.append("chest_pressure_on_exertion")
    return uniq[:132]
